...
Logs::Logs(std::string filename)
{
    ...
    if (filedes.is_open())
    {
        while (filedes.good())
        {
            ...
            args = parse_log(line);
            track_boss(args);
            if (args[2] == "SPELL_AURA_APPLIED" && atoi(args[11].c_str()) == 110660)
            {
...
void    Logs::track_boss(std::deque<std::string> args)
{
    t_boss  tab[] =
    {
        ...
        {"Tendons brûlants", "Burning Tendons", "Echine d'Aile de Mort", "Spine of Deathwing"},
...
void    Logs::track_potions(std::deque<std::string> args)
{
    ...
    while (idx < 5)
    {
        if (my_potions[idx].item_id == atoi(args[11].c_str()))
        {
            ...
            {
                std::cout << ji << ": " << args[4] << " a utilisé une potion " << my_potions[idx].type << " sur le boss " << this->actual_boss << std::endl;
                ji++;
            }
            else
            {
                std::cout << ji << ": " << args[4] << " a prepote " << my_potions[idx].type << std::endl;
                ji++;
            }
        }
        idx++;
